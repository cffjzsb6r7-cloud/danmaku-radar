...
GitHub Actions 每 24 小时（每天 08:00 北京时间）自动执行一次：抓取 B站真实热榜 → 更新 SQLite 数据库 → 提交数据快照 → 重新发布 GitHub Pages；后端部署在 Render，保证网站与接口始终可用。
